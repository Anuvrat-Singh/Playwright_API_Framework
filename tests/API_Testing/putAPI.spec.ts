 ...
 */
test("PUT request: Update booking details after creating a booking", async ({
  request,
}) => {
  ...
  const putAPIResponse_JSON = await putAPIResponse.json();
  console.log(
    "PUT API response : " + JSON.stringify(putAPIResponse_JSON, null, 2)
  );
});
